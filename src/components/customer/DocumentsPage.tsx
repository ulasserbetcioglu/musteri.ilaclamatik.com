      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            ...
            <span>Belgeleriniz yükleniyor...</span>
          </div>
        ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {msdsDocuments.length === 0 ? (
                  <div className="col-span-full bg-white rounded-2xl p-12 text-center border-2 border-dashed border-gray-200">
                    ...
                    <p className="text-gray-500">Henüz kayıtlı belge bulunamadı.</p>
                  </div>
                ) : (
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedImage(null)}>
          ...
          <div className="max-w-5xl w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-4 p-5 bg-white rounded-xl flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl">
              <div>
                <h3 className="font-bold text-gray-900 text-lg">{selectedImage.title}</h3>
                <p className="text-sm text-gray-500">{new Date(selectedImage.created_at).toLocaleDateString('tr-TR')}</p>
              </div>
              <div className="flex gap-2 w-full md:w-auto">
                <a href={selectedImage.file_url} target="_blank" rel="noopener noreferrer" className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors">
                  <Eye size={16} /> Tam Ekran Aç
                </a>
                <a href={selectedImage.file_url} download className="flex-1 px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-2 hover:bg-black transition-colors">
