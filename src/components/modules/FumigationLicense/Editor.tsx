...
export function FumigationLicenseEditor({ settings, onSettingsChange }: FumigationLicenseEditorProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-green-50 p-3 rounded border border-green-200 text-sm text-green-800 mb-4">
        Fumigasyon ruhsatı PDF dosyalarını buradan yönetin. Eklenen PDF'ler tüm müşteriler için geçerlidir.
      </div>
      ...
      <section>
        ...
        <div className="space-y-3 p-3 bg-gray-50 rounded border">
          <input
            type="text"
            placeholder="PDF Başlığı (ör: Fumigasyon Ruhsatı 2024)"
            value={newPdfTitle}
          ...
          <input
            type="text"
            placeholder="PDF URL (ör: /documents/fumigasyon-ruhsati.pdf)"
            value={newPdfUrl}
      ...
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wider mb-3 flex items-center gap-2 border-t pt-4" style={{ color: BRAND_GREEN }}>
          <ShieldCheck size={16} /> Yüklü PDF Dosyaları ({pdfs.length})
        </h2>
